...
class CrudApiCommand extends Command
{
    ...
    protected $signature = 'crud:api
                            ...
                            {--table= : Nome da porra da tabela.}
							{--timestamps=yes : Include option timestamps.}
    ...
    public function handle()
    {
		...
		$tableName = empty($table) ? $tableName : $table;
		
        $this->call('crud:api-controller', ['name' => $controllerNamespace . $name . 'Controller', '--crud-name' => $name, '--model-name' => $modelName, '--model-namespace' => $modelNamespace, '--pagination' => $perPage, '--validations' => $validations, '--pk' => $primaryKey]);
