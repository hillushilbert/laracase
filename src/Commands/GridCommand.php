...
class GridCommand extends Command
{
     ...
     * @var string
     */
    protected $signature = 'grid:generate
                            {name : The name of the Crud.}
                            {--fields= : Field names for the form & migration.}
                            {--fields_from_file= : Fields from a json file.}
                            {--validations= : Validation rules for the fields.}
                            {--controller-namespace= : Namespace of the controller.}
                            {--model-namespace=Models : Namespace of the model inside "app" dir.}
                            ...
                            {--form-helper=grid : Helper for generating the form.}
                            ...
                            {--grid-fields= : The amount of models per page for index pages.}
                            {--buttons=S : Adiciona botoes ao datatable.}
                            {--group-fields= : Lista de campos agrupados. }
                            {--use-tabs=no : Se o cadastro vai usar abas.}
                            {--tab-fields= : Quais são os campos por tabs. }                            
                            {--timestamps=yes : Include soft deletes fields.}';
    ...
    public function handle()
    {
        ...
        $groupFields = $this->option('group-fields');

        $routeGroup = $this->option('route-group');
        $tableName = !empty($this->option('table')) 
					 ?$this->option('table')
					 :$migrationName;
        ...
        $buttons = trim($this->option('buttons')); 
        ...
        $gridFields = $this->option('grid-fields');

        $indexes = $this->option('indexes');
        $relationships = $this->option('relationships');
        if ($this->option('fields_from_file') && empty($relationships)) {
        ...
        $useTabs = ($this->option('use-tabs')) ? trim($this->option('use-tabs'))  : 'no';
        $tabFields = ($this->option('tab-fields')) ? trim($this->option('tab-fields'))  : null;


        $this->call('grid:controller', ['name' => $controllerNamespace . $name . 'Controller', '--crud-name' => $name, '--model-name' => $modelName, '--model-namespace' => $modelNamespace, '--view-path' => $viewPath, '--route-group' => $routeGroup, '--pagination' => $perPage, '--fields' => $fields, '--validations' => $validations, '--auth' => $auth, '--primary-key' => $primaryKey]);
        ...
        $this->call('grid:view', ['name' => $name, '--fields' => $fields, '--validations' => $validations, 
            '--view-path' => $viewPath, '--route-group' => $routeGroup, '--localize' => $localize, 
            '--pk' => $primaryKey, '--form-helper' => $formHelper, '--grid-fields' => $gridFields, 
            '--buttons'=>$buttons,'--group-fields'=>$groupFields,
            '--use-tabs'=> $useTabs, '--tab-fields' => $tabFields,
        ]);
        if ($localize == 'yes') {
    ...
    protected function processJSONFields($file)
    {
        ...
        foreach ($fields->fields as $field) {
            if ($field->type === 'select' || $field->type === 'enum' || $field->type === 'radio') {
